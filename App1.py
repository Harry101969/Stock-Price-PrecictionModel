...
prediction_log_filename = f"{stock}_prediction_log.csv"
...
if os.path.exists(prediction_log_filename):
    # Append to existing file
    existing_data = pd.read_csv(prediction_log_filename)
    updated_data = pd.concat([existing_data, pd.DataFrame([prediction_record])], ignore_index=True)
    updated_data.to_csv(prediction_log_filename, index=False)
else:
    # Create new file
    pd.DataFrame([prediction_record]).to_csv(prediction_log_filename, index=False)

st.subheader("✅ Prediction Log")
st.success(f"Prediction record saved to {prediction_log_filename} at {prediction_timestamp}")

# Download prediction log
with open(prediction_log_filename, 'rb') as file:
    st.download_button(
