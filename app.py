...
def create_prediction_record(record_date, actual_price, predicted_price, is_current_search=False):
    ...
    # For current searches, use the actual current time
    if is_current_search:
        timestamp = dt.datetime.now()
    # For historical records, use 8:00 AM on that date
    elif isinstance(record_date, (pd.Timestamp, dt.datetime)):
...
if len(df) >= 130:  # Need at least 100 + 30 days
    ...
    for date, actual, pred in zip(historical_dates, historical_actual_prices, historical_pred_prices):
        if not np.isnan(pred):
            historical_records.append(create_prediction_record(date, actual, pred, is_current_search=False))

# Create current prediction record with current time
current_record = create_prediction_record(current_date, current_price, top_prediction, is_current_search=True)
