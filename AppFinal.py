...
# # 🚀 Predict the next 10 dayss recursively (~10 days)
...
# # Display table
# st.subheader("Predicted Prices (Current Day + Next 5 Days)")
# st.write(target_df)
...
# # Use current day prediction instead of next day
# profit_loss = (current_day_prediction - current_price) * shares_bought

# if profit_loss > 0:
#     st.success(f"🎉 Expected **Profit** for current day: **{currency_symbol}{profit_loss:.2f}**")
# elif profit_loss < 0:
#     st.error(f"⚠️ Expected **Loss** for current day: **{currency_symbol}{profit_loss:.2f}**")
# else:
#     st.info("No gain or loss expected for the current day.")
